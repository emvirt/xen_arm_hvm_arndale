...
start:
    ...
    if (vcpu_to_pcpu) {
    	...
    	//printf("==== Before libxl_cpu_bitmap_alloc function !!  =====\n");	//added by Tae
        ret = libxl_cpu_bitmap_alloc(ctx, &vcpu_cpumap, 0);
    	//printf("==== After libxl_cpu_bitmap_alloc function !!  =====\n");	//added by Tae
        if (ret){
    	    //printf("==== error with libxl_cpu_bitmap_alloc function !!  =====\n");	//added by Tae
            goto error_out;
	}
    	...
    	//printf("==== Before libxl_bitmap_dispose function !!  =====\n");	//added by Tae
        libxl_bitmap_dispose(&vcpu_cpumap);
    ...
    //printf("==== Before release_lock function !!  ====\n");	//added by Tae
    release_lock();

    if (!paused){
    
    	//printf("=== Domain was paused, so unpause domain  ====\n");	//added by Tae
        libxl_domain_unpause(ctx, domid);
    }
    ret = domid; /* caller gets success in parent */
    if (!daemonize && !monitor){
    	
    	//printf("=== Daemonize and monitor  ====\n");	//added by Tae
	goto out;
    }
    if (dom_info->vnc)
    ...
    if (need_daemon) {
    	...
    	//printf("===== Before xl_fork function !!!  =====\n");	//added by Tae
	...
	//printf("===== Before libxl_create_logfile !!! =====\n");	//added by Tae
    ...
    //printf("====== Before libxl_evenable_domain_death !!!  =======\n");	//added by Tae
    ...
    //printf("==== Before domain_wait_event function !! =====\n");	//added by Tae
    while (1) {
        libxl_event *event;
...
{
    printf("===== libxl_domain_pause function was called !!! =====\n");
    libxl_domain_pause(ctx, domid);
}

static void unpause_domain(uint32_t domid)
{
    printf("===== libxl_domain_unpause function was called !!! =====\n");
    libxl_domain_unpause(ctx, domid);
}
                            ...
                            int send_fd, int recv_fd, int remus)
{
    ...
    printf("====== migrate_receive function was called ======");	//added by Tae
