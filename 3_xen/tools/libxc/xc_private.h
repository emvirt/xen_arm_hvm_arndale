...
static inline int do_domctl(xc_interface *xch, struct xen_domctl *domctl)
{
    ...
    {
        //DPRINTF("--------------- xc_hypercall_bounce_pre return value was 1 ---------------- \n");
        //DPRINTF("--------------- goto out1 !!!! ---------------- \n");
        PERROR("Could not bounce buffer for domctl hypercall");
        goto out1;
    }
    //DPRINTF("--------------- After xc_hypercall_bounce_pre function ---------------- \n");
    ...
    if ( (ret = do_xen_hypercall(xch, &hypercall)) < 0 )
    {
    	//DPRINTF("--------------- do_xen_hypercall return value, ret < 0 ---------------- \n");
        if ( errno == EACCES )
            DPRINTF("domctl operation failed -- need to"
                    " rebuild the user-space tool set?\n");
    }
    //DPRINTF("--------------- After do_xen_hypercall function ! ---------------- \n");
    //DPRINTF("--------------- ret 2 : %d ---------------- \n",ret);

    xc_hypercall_bounce_post(xch, domctl);
    //DPRINTF("--------------- ret 3 : %d ---------------- \n",ret);
    //DPRINTF("--------------- After xc_hypercall_bounce_post function ! ---------------- \n");
 out1:
