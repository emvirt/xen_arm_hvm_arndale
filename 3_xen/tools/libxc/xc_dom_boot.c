...
static int setup_hypercall_page(struct xc_dom_image *dom)
{
    ...
    //printf("======= setup_hypercall_page function was called !!! =======\n");	//added by Tae

    if ( dom->parms.virt_hypercall == -1 ){
     	//printf("virt_hypercall value was : -1 \n");	//added by Tae
	return 0;
    }
    
    //printf("dom->parms.virt_hypercall : 0x%" PRIx64  " \n",dom->parms.virt_hypercall);	//added by Tae
    //printf("dom->parms.virt_base : 0x%" PRIx64  " \n",dom->parms.virt_base);	//added by Tae
    //printf("== XC_DOM_PAGE_SHIFT return value : %d ==\n", (dom)->arch_hooks->page_shift);	//added by Tae
    
    pfn = (dom->parms.virt_hypercall - dom->parms.virt_base) >> XC_DOM_PAGE_SHIFT(dom);

    //pfn = (dom->parms.virt_hypercall - dom->parms.virt_base) << XC_DOM_PAGE_SHIFT(dom);	//added by Tae
 
    //pfn = (0x80008000 - 0x80000000);	//added by Tae	 
 
    //printf("=== virt_hypercall - virt_base : 0x%" PRIx64 " === \n",
    //					dom->parms.virt_hypercall - dom->parms.virt_base);	//added by Tae
    
    //pfn = 0xb0000000;	//added by Tae

    //printf("pfn : 0x%" PRIpfn  " \n",pfn);	//added by Tae
    //printf("pfn : %llu" PRIu32 " \n",pfn);	//added by Tae

    DOMPRINTF("%s: vaddr=0x%" PRIx64 " pfn=0x%" PRIpfn "", __FUNCTION__,
                  dom->parms.virt_hypercall, pfn);
    
    domctl.cmd = XEN_DOMCTL_hypercall_init;	// 22
    domctl.domain = dom->guest_domid;
    
    //printf("====== dom->guest_domid : %d =====\n",dom->guest_domid);	//added by Tae
    //printf("============= Before xc_dom_p2M_guest function =============\n");	//added by Tae
    
    domctl.u.hypercall_init.gmfn = xc_dom_p2m_guest(dom, pfn);

    //printf("===== domctl.u.hypercall_init.gmfn : 0x%" PRIpfn "=====\n",domctl.u.hypercall_init.gmfn);	//added by Tae

    //printf("============= Before do_domctl function =============\n");	//added by Tae
    
    rc = do_domctl(dom->xch, &domctl);

    //printf("============= After do_domctl function =============\n");	//added by Tae
    //printf("============= rc : %d =============\n",rc);	//added by Tae
    if ( rc != 0 )
