 ...
 * Added by Taekyoung
 * TIMER_MONITOR : 
 * Collect timer interval values to measure domain's utilization 
 * 
 */
...
#define XEN_TIMER_MONITOR_SIZE 10000
#define XEN_TIMER_MONITOR_ROW_SIZE 4

uint32_t xen_time_monitor_flag = 0;

uint32_t xen_column_index = 0;
uint32_t xen_row_index = 0;
uint32_t reprogram_index = 0;

uint32_t ptime_high = 0;
uint32_t ptime_low = 0;


uint32_t xen_timer_monitor_table[XEN_TIMER_MONITOR_SIZE];
uint64_t xen_reprogram_timer_table[XEN_TIMER_MONITOR_SIZE];
//uint32_t xen_timer_monitor_table[XEN_TIMER_MONITOR_SIZE][XEN_TIMER_MONITOR_ROW_SIZE];
...
int reprogram_timer(s_time_t timeout)
{
    ...
    if((xen_time_monitor_flag == 1)&& (reprogram_index < XEN_TIMER_MONITOR_SIZE)){
        xen_reprogram_timer_table[reprogram_index] = deadline;
        reprogram_index++;
    }
...
/*
    if((xen_time_monitor_flag == 1) && (xen_row_index < XEN_TIMER_MONITOR_SIZE)){

    	__asm("MRRC p15, 0, %0, %1, c14":"=r"(ptime_low),"=r"(ptime_high):);

	xen_timer_monitor_table[xen_row_index][xen_column_index] = ptime_low;
	
	xen_column_index++;
	
	if(xen_column_index > XEN_TIMER_MONITOR_ROW_SIZE){
...
/*
	...
	//timer_monitor_table[timer_tick_index][0] = ptime_high;
	xen_timer_monitor_table[xen_row_index] = ptime_low;
