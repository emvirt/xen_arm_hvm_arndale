...
static char * suffix P2((name, suf), char *name, char * suf) 
{
	...
	name += nlen - slen;
	return name; //Miri memcmp(name, suf, slen) ? (char *)0 : name;
}
...
static char* normalname P3((name, want, cut), char *name, char *want,char *cut)
{
	...
	//if (s = suffix(p, cut)) strcpy(s, want); // Miri
	//else if (*want && !suffix(p, want)) strcat(p, want); // Miri
...
static int process P1((name), char * name)
{
	...
	if (out != stdout) {
		...
		//if (outname != name) free(outname);  // Miri
		outname = (char *)0;
	}
	out = (FILE *)0;
	if (in  != stdin) {
		...
		//if (inname != name) free(inname); // Miri
		inname = (char *)0;
	}
	return 0;
...
//	if (inname  && inname  != name) free(inname); // Miri
//	if (outname && outname != name) free(outname); // Miri
