...
//	printf("[init_page_allocator] start: min=%x, max=%x\n", min, max);
...
#else
unsigned int YSB_allocator_index = 0;
unsigned long alloc_pages(int order) {
/*	unsigned long pfn_to_map = PFN_UP(PHYS_OFFSET) + (start_info.nr_pt_frames - 1) * L1_PAGETABLE_ENTRIES;
	unsigned long start_address = (unsigned long)pfn_to_virt(pfn_to_map);
	unsigned long result = (((start_address)>>PAGE_SHIFT)+YSB_allocator_index)<<PAGE_SHIFT;

	if (order>0) {
		int i;
		for (i=0; i<order;i++)
			YSB_allocator_index += i;
	} else YSB_allocator_index++;
...
#if 0
void create_io_mapping(unsigned long virt_addr, unsigned long phys_addr, unsigned long size)
{   
	...
	mmu_update_t mmu_updates[1];
        ...
        pde = (unsigned long *)start_info.pt_base  + (virt_addr>>20);
        va = virt_addr;
        pa = phys_addr;
        end = pa + size;
        ...
        if(((va | pa | end) & ~SECTION_MASK) == 0) {
                ...
                flags |= PDE_AP_SRW_URW;
                do {
                        //*pde = (pa & SECTION_MASK) | flags;
			mmu_updates[0].ptr = (unsigned long)pde;
			mmu_updates[0].val = (pa & SECTION_MASK) | flags ;
			mmu_updates[0].flags = (unsigned long)to_virt(mmu_updates[0].ptr);//YSB
			
			if(HYPERVISOR_mmu_update(mmu_updates, 1, NULL, DOMID_SELF) < 0)
    			{
         			printk("create_io_mapping: PTE for new page table page could not be updated\n");
         			*((unsigned long *)0) = 0;
    			}
...
//    		 level, *pt_pfn, prev_l_mfn, offset);
	...
	*((unsigned long *)0)=0;
    ...
    mmu_updates[0].flags = (unsigned long)to_virt(mmu_updates[0].ptr);//YSB
//YSB
/*    printk("l1_table_offset(%x) = %x\n",pt_page, l1_table_offset(pt_page));
    printk("l2_table_offset(%x) = %x\n",pt_page, l2_table_offset(pt_page));
    printk("tab[l2_table_offset(%x)] /%x/ = %x\n",pt_page, &tab[l2_table_offset(pt_page)],tab[l2_table_offset(pt_page)]);*/

    if(HYPERVISOR_mmu_update(mmu_updates, 1, NULL, DOMID_SELF) < 0)
    {
         printk("new_pt_frame: PTE for new page table page could not be updated\n");
         *((unsigned long *)0) = 0;
    }
...
//		   virtaddr, physaddr, count);
	...
	for( i = 0; i < count; i++ )
	{
		...
		offset = l2_table_offset(vaddr) - 1; //YSB 
		l2pte = l2tab[offset];
...
//			printf("[update_pagetable] l1tab_mfn = %x\n", l1tab_mfn);

			memset((unsigned long*)pfn_to_virt(l1tab_mfn), 0, PAGE_SIZE);  
			...
			if(HYPERVISOR_mmu_update(mmu_updates, 1, NULL, DOMID_SELF) < 0)
			{
				printk("update_pagetable: PTE for new page table page could not be updated\n");
				printk("mmu_updates.ptr %llx %llx \n", mmu_updates[0].ptr, mmu_updates[0].val);
				*((unsigned long *)0) = 0;
			}
		...
		mmu_updates[i].flags = (unsigned long)to_virt(mmu_updates[i].ptr);//YSB
...
//    pfn_to_map = (start_info.nr_pt_frames - 1) * L1_PAGETABLE_ENTRIES;
    pfn_to_map = PFN_UP(PHYS_OFFSET) + (start_info.nr_pt_frames - 1) * L1_PAGETABLE_ENTRIES;//YSB
//    pfn_to_map = PFN_UP(to_phys(start_info.pt_base));
	...
	// shyoo
	//end_address = start_address + 0x2*PAGE_SIZE;
    while(start_address < end_address && pfn_to_map < *max_pfn)
    {
	//unsigned long flags;
	// shyoo
	//printk("start_address: %x, end_address: %x\n", start_address, end_address);
	...
	// shyoo
	//ptr = &tab[offset];
	//printk("tab: %x, mfn: %x, offset: %x, ptr: %x *ptr: %x\n", tab, mfn, offset, ptr, *ptr);
...
//			   pfn_to_map, pfn_to_mfn(pfn_to_map));
	...
	// shyoo
	//printk("page=tab[offset]: %x, mfn=pte_to_mfn(page): %x, offset: %x\n", page, mfn, offset);

        mmu_updates[count].ptr = (mfn << PAGE_SHIFT) + sizeof(void *) * offset;
	temp = (pfn_to_mfn(pfn_to_map++) << PAGE_SHIFT) | __L1_PAGE_USER_SMALL;
        mmu_updates[count].val = (uint64_t)temp;
	mmu_updates[count].flags = (unsigned long)to_virt(mmu_updates[count].ptr);//YSB
	// shyoo
	//printk("pfn_to_map:%x, __L1_PAGE_USER_SMALL: %x temp: %x\n", pfn_to_map, __L1_PAGE_USER_SMALL, temp);
	//printk("count:%x mmu_update.ptr: %x, mmu_update.val: %x\n", count, mmu_updates[count].ptr, mmu_updates[count].val);
        count++;
...
void mm_init(void)
{
	...
	max_pfn = PFN_UP(PHYS_OFFSET) + start_info.nr_pages;//YSB
//	max_pfn = start_info.nr_pages;
	// shyoo
//	printk("start_pfn: %x max_pfn: %x\n", start_pfn, max_pfn);
//	printk("to_phys(start_info.pt_base(%x))(%x)\n", start_info.pt_base, to_phys(start_info.pt_base));
//	printk(" + start_info.nr_pt_frames(%x) + 3\n", start_info.nr_pt_frames);
