...
#define PHYSICAL_TIME 0
//#define PHYSICAL_TIME 1
...
#define IRQ_LATENCY_TABLE_SIZE 10000
INT32U latency_table_index = 0;
INT32U irq_latency_table[IRQ_LATENCY_TABLE_SIZE];
#endif



extern time_table_index;
extern t1_miss_count;
extern t2_miss_count;
extern loop_count;
...
void  OSInit (void)
{
    ...
    //printk("init task idle\n");
    OS_InitTaskIdle();                                           /* Create the Idle Task                     */
    //printk("init task stat\n");
#if OS_TASK_STAT_EN > 0
...
INT32U init_physical_time = 0;//YSB
...
void  OSTimeTick (void)
{
    ...
    //Physical_Time = *(int*)(0x7000E008)*100+*(int*)(0x7000E010)/10;//for tegra3//++;//for Software// = HYPERVISOR_shared_info->ticks - init_physical_time;//for Xen//YSB

    // update OSTime
   
    OSTime++;
#ifdef PROFILE_IRQ_LATENCY
	__asm("mrc p15, 0, %0, c9, c13, 0":"=r"(ccnt):);
	if(latency_table_index < IRQ_LATENCY_TABLE_SIZE) {
		irq_latency_table[latency_table_index] = ccnt;
		latency_table_index++;
	}
	else {
		//Generate Software Breakpoint to dump profile data
		__asm("BKPT");
	}
#endif	
	// shyoo
	//printk("OSTimeTick(%d) : OSPrioHighRdy %d OSPrioCur %d\n", OSTime, OSPrioHighRdy, OSPrioCur);
//	witak
    //OSTime++;
  //      Physical_Time = ((struct vcpu_time_info *)&HYPERVISOR_shared_info->vcpu_info[0].time)->system_time & 0xFFFFFFFF;
	...
	while (ptcb->OSTCBPrio != OS_TASK_IDLE_PRIO) {     /* Go through all TCBs in TCB list              */
		//OS_ENTER_CRITICAL(); //SWKIM
						 
	//	if (ptcb->OSTCBDly != 0) {                     /* No, Delayed or waiting for event with TO     */
	//		if (--ptcb->OSTCBDly == 0) {               /* Decrement nbr of ticks to end of delay       */
												   /* Check for timeout                            */
	
		//if (ptcb->OSTCBWake != 0) {                     /* No, Delayed or waiting for event with TO     */
		if (ptcb->OSTCBDly != 0) {                     /* No, Delayed or waiting for event with TO     */
			//if ( ptcb->OSTCBWake <= OSTime)
			// Wake up logic 
			// if ( ptcb->OSTCBDly <= OSTime)
#if PHYSICAL_TIME
			ptcb->OSTCBDly -= physical_time_offset;
#else
			ptcb->OSTCBDly--;	
#endif
			if (ptcb->OSTCBDly <= 0)
...
void  OS_Sched (void)
{
	...
	//printk("OS_Sched ( %d ) OSTCBCur->OSTCBDly %d \n", OSTime, OSTCBCur->OSTCBDly); 
	//printk("OS_Sched (%d) : OSPrioHighRdy %d OSPrioCur %d\n", OSTime, OSPrioHighRdy, OSPrioCur);
    OS_ENTER_CRITICAL();
...
void  OSPullDelay (void)
{
	...
	pullDly = ptcb->OSTCBDly;

        while (checkTCB->OSTCBPrio != OS_TASK_IDLE_PRIO) 
	{     
	    if(pullDly > checkTCB->OSTCBDly)
		pullDly = checkTCB->OSTCBDly;
//	    printk("pullDly::%d,OSTCBDly::%d\n",pullDly,checkTCB->OSTCBDly);
            checkTCB = checkTCB->OSTCBNext;                
	}
	//local_irq_disable();

#if XEU
//	if(OSTime >Physical_Time)
//		printk("Watch::OSTime::%d,Physical_Time::%d\n",OSTime,Physical_Time);

	if(pullDly + OSTime >= Physical_Time)
		pullDly = (Physical_Time - OSTime);
//	else
//		printk("WOW::OSTime::%d,Physical_Time::%d\n",OSTime,Physical_Time);
//	if(pullDly)
//		printk("jjw pullDly:%d \n",pullDly);
	    ...
	    if(pullDly != 0 && ptcb->OSTCBDly != 0) {
		OS_ENTER_CRITICAL();
		ptcb->OSTCBDly -= pullDly;
...
void  OS_TaskIdle (void *p_arg)
{
	...
	if (Time_ != last_time)
        {
		//printk("jjw idle task :virtual %d Physical %d\n",OSTime,Physical_Time); //OSIdleCtr);
		last_time = Physical_Time;
	}
...
// shyoo
extern INT32S	total_count, fail_count;
...
void  OS_TaskStat (void *p_arg)
{
    ...
    for (;;) {
        ...
        //time = OSTimeGet();
        //printk("Task stat time : %d OSCPUUsage : %d run : %d\n", time, OSCPUUsage, run);
        //printk("Task stat OSCPUUsage : %d run : %d success rate:count(%d %d/%d)=%d \n", 
	//	OSCPUUsage, run, total_count, fail_count, total_count, ((total_count + fail_count)*100)/total_count);
        OSTaskStatHook();                        /* Invoke user definable hook                         */
#if (OS_TASK_STAT_STK_CHK_EN > 0) && (OS_TASK_CREATE_EXT_EN > 0)
        OS_TaskStatStkChk();                     /* Check the stacks for each task                     */
#endif
        //HYPERVISOR_do_print_profile();
...
void PrintTaskContext(unsigned int *  sp )
{
  int i;
  unsigned int *ptr = sp;
/*  for(i=0; i<15; i++)
