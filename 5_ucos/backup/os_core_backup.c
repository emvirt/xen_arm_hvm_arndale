...
#ifdef CONFIG_XEN
void UpdateOSTimeXen()
{
  u32 last_jiffies = current_jiffies;
  
  struct vcpu_time_info *src = &HYPERVISOR_shared_info->vcpu_info[0].time;
  current_jiffies = src->freetimer_overflows;
  
//  OSTime = OSTime + (current_jiffies - last_jiffies);
  
  printk("last : %x current : %x OSTime : %x\n", last_jiffies, current_jiffies, OSTime);
...
#if jjw
  	volatile int64_t systime;
  	INT32U systime_offset;
  	INT32U OSTime_;
	INT32S pushtime;

  	systime = ((struct vcpu_time_info *)&HYPERVISOR_shared_info->vcpu_info[0].time)->system_time;
	systime_offset = systime & 0xFFFFFFFF;
	OSTime_ = OSTimeGet();

	pushtime = pullDly;	
	if( (pullDly + OSTime_) > systime_offset )
		pullDly = (systime_offset -OSTime_  );
//	printk("systime_offset %d OSTime %d pullDly %d ptcb->OSTCBDly %d\n", systime_offset, OSTime_, pullDly, ptcb->OSTCBDly);
	//pullDly = 0;
#endif	
	...
	if(pullDly)
	{
		...
		OSTime += pullDly;
#else
//		printk("systime_offset %d OSTime %d pullDly %d \n", systime_offset, OSTime_, pullDly);
