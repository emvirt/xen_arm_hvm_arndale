...
CPU_BOOLEAN  NetARP_IsAddrProtocolConflict (void)
{
    ...
    NetARP_HostAddrProtocolConflict = DEF_NO;                       /* Reset addr conflict flag (see Note #1b).         */
    CPU_CRITICAL_EXIT();
...
*                   (b) Configure target's hardware address as NULL
*                   (c) Configure sender's protocol address as this host's protocol address
*                   (d) Configure target's protocol address as this host's protocol address     See Note #4a
...
*
*                   See also 'NetARP_IsAddrProtocolConflict()  Note #1'
...
*               called by application function(s).
*
* Note(s)     : (3) NetARP_TxReqGratuitous() blocked until network initialization completes.
*
*               (4) RFC #3927, Section 2.4 states that one purpose for transmitting a gratuitous ARP Request
    ...
    if (Net_InitDone != DEF_YES) {                              /* If init NOT complete, exit tx (see Note #3).         */
       *perr = NET_ERR_INIT_INCOMPLETE;
        return;
                                                                ...
                                                                /* Cfg ARP Req from host addr (see Note #1).            */
...
*
*               (4) (a) RFC # 826, Section 'Related issue' states that "perhaps receiving of a packet from
*                       a host should reset a timeout in the address resolution entry".
*
