...
void  NetIP_Init (void)
{
    ...
    NetIP_StatRxDestLocalHostCtr        =  0;
    ...
    NetIP_StatTxDestThisHostCtr         =  0;
    NetIP_StatTxDestLocalHostCtr        =  0;
    NetIP_StatTxDestLocalLinkCtr        =  0;
    NetIP_StatTxDestLocalNetCtr         =  0;
    NetIP_StatTxDestRemoteNetCtr        =  0;
...
* Argument(s) : none.
*
* Return(s)   : This host's IP address subnet mask in host-order.
*
* Caller(s)   : Application.
*
*               This function is a network protocol suite application interface (API) function & MAY be 
...
NET_IP_ADDR  NetIP_GetAddrSubnetMask (void)
{
    ...
    CPU_CRITICAL_ENTER();
    addr = NetIP_AddrThisHostSubnetMask;
    CPU_CRITICAL_EXIT();
...
*
*
* Argument(s) : addr        IP address to validate.
*
...
CPU_BOOLEAN  NetIP_IsAddrClassA (NET_IP_ADDR  addr)
{
    ...
    is_addr = ((addr & NET_IP_ADDR_CLASS_A_MASK) == NET_IP_ADDR_CLASS_A) ? DEF_YES : DEF_NO;
...
CPU_BOOLEAN  NetIP_IsAddrClassB (NET_IP_ADDR  addr)
{
    ...
    is_addr = ((addr & NET_IP_ADDR_CLASS_B_MASK) == NET_IP_ADDR_CLASS_B) ? DEF_YES : DEF_NO;
...
CPU_BOOLEAN  NetIP_IsAddrClassC (NET_IP_ADDR  addr)
{
    ...
    is_addr = ((addr & NET_IP_ADDR_CLASS_C_MASK) == NET_IP_ADDR_CLASS_C) ? DEF_YES : DEF_NO;
...
CPU_BOOLEAN  NetIP_IsAddrThisHost (NET_IP_ADDR addr)
{
    ...
    is_addr = (addr == NET_IP_ADDR_THIS_HOST) ? DEF_YES : DEF_NO;
...
CPU_BOOLEAN  NetIP_IsAddrLocalHost (NET_IP_ADDR  addr)
{
    ...
    is_addr = ((addr & NET_IP_ADDR_LOCAL_HOST_MASK_NET) == NET_IP_ADDR_LOCAL_HOST_NET) ? DEF_YES : DEF_NO;
...
CPU_BOOLEAN  NetIP_IsAddrLocalLink (NET_IP_ADDR  addr)
{
    ...
    is_addr = ((addr >= NET_IP_ADDR_LOCAL_LINK_HOST_MIN) &&
...
CPU_BOOLEAN  NetIP_IsAddrBroadcast (NET_IP_ADDR  addr)
{
    ...
    is_addr = (addr == NET_IP_ADDR_BROADCAST) ? DEF_YES : DEF_NO;
...
*                       (4) Specified Host                              RFC #1122, Section 3.2.1.3.(b)
...
CPU_BOOLEAN  NetIP_IsValidAddrHost (NET_IP_ADDR  addr_host)
{
    ...
    if (addr_host == NET_IP_ADDR_THIS_HOST) {
        valid = DEF_NO;
                                                                ...
                                                                /* Chk invalid lim'd broadcast         (see Note #1a2). */
        ...
        valid = DEF_NO;
                                                                ...
                                                                /* Chk invalid link-local 'This Host'  (see Note #1a4). */
        ...
        if ((addr_host < NET_IP_ADDR_LOCAL_LINK_HOST_MIN) ||
            (addr_host > NET_IP_ADDR_LOCAL_LINK_HOST_MAX)) {
             valid = DEF_NO;
                                                                    ...
                                                                    /* Chk invalid Class-A 'This Host' (see Note #1a1). */
        ...
        if ((addr_host             & NET_IP_ADDR_CLASS_A_MASK_HOST) ==
            (NET_IP_ADDR_BROADCAST & NET_IP_ADDR_CLASS_A_MASK_HOST)) {
             valid = DEF_NO;
                                                                    ...
                                                                    /* Chk invalid Class-B 'This Host' (see Note #1a1). */
        ...
        if ((addr_host             & NET_IP_ADDR_CLASS_B_MASK_HOST) ==
            (NET_IP_ADDR_BROADCAST & NET_IP_ADDR_CLASS_B_MASK_HOST)) {
             valid = DEF_NO;
                                                                    ...
                                                                    /* Chk invalid Class-C 'This Host' (see Note #1a1). */
...
*
* Description : (1) Validate this host's IP address :
*
*                   (a) MUST NOT be one of the following :
*
*                       (1) This      Host                              RFC #1122, Section 3.2.1.3.(a)
...
* Argument(s) : addr_host           IP host address to validate.
*
*               addr_subnet_mask    IP      address subnet mask.
*
                                        ...
                                        NET_IP_ADDR  addr_subnet_mask)
{
    ...
    if (valid == DEF_YES) {
                                                                    /* Chk invalid subnet 'This Host' (see Note #1a1).  */
...
*
* Argument(s) : addr_subnet_mask    IP address subnet mask to validate.
*
...
*
*                           (B) (1)                           MUST   NOT be one of the following :
...
*                                   (a) This      host's                    RFC #1122, Section 3.2.1.3.(1)
...
/*$PAGE*/
    ...
    NET_UTIL_VAL_COPY_GET_NET_32(&pbuf_hdr->IP_AddrSrc, &pip_hdr->AddrSrc);
                                                                    ...
                                                                    /* Chk invalid localhost       (see Note #9c1B2a).  */
    if ((pbuf_hdr->IP_AddrSrc & NET_IP_ADDR_LOCAL_HOST_MASK_NET) == NET_IP_ADDR_LOCAL_HOST_NET) {
        NET_CTR_ERR_INC(NetIP_ErrRxHdrAddrSrcCtr);
       *perr = NET_IP_ERR_INVALID_ADDR_SRC;
        return;

                                                                    /* Chk invalid lim'd broadcast (see Note #9c1B1A).  */
       ...
       *perr = NET_IP_ERR_INVALID_ADDR_SRC;
        return;
                                                                    ...
                                                                    /* Chk link-local broadcast    (see Note #9c1B1b).  */
        ...
        if ((pbuf_hdr->IP_AddrSrc < NET_IP_ADDR_LOCAL_LINK_HOST_MIN) ||
            ...
            *perr = NET_IP_ERR_INVALID_ADDR_SRC;
             return;
                                                                    ...
                                                                    /* Chk Class-A broadcast       (see Note #9c1B1b).  */
        if ((pbuf_hdr->IP_AddrSrc  & NET_IP_ADDR_CLASS_A_MASK_HOST) ==
            ...
            *perr = NET_IP_ERR_INVALID_ADDR_SRC;
             return;
                                                                    ...
                                                                    /* Chk Class-B broadcast       (see Note #9c1B1b).  */
        if ((pbuf_hdr->IP_AddrSrc  & NET_IP_ADDR_CLASS_B_MASK_HOST) ==
            ...
            *perr = NET_IP_ERR_INVALID_ADDR_SRC;
             return;
                                                                    ...
                                                                    /* Chk Class-C broadcast       (see Note #9c1B1b).  */
       ...
       *perr = NET_IP_ERR_INVALID_ADDR_SRC;
        return;
                                                                    ...
                                                                    /* Chk subnet  broadcast       (see Note #9c1B1c).  */
...
/*$PAGE*/
    ...
    if (pbuf_hdr->IP_AddrDest  == NetIP_AddrThisHost) {             /* Chk this host's addr       (see Note #9d1B1a).   */
       ...
       *perr = NET_IP_ERR_INVALID_ADDR_DEST;
        ...
        NET_CTR_STAT_INC(NetIP_StatRxDestLocalHostCtr);
...
/*$PAGE*/
        ...
        if (pbuf_hdr->IP_AddrDest != NET_IP_ADDR_BROADCAST) {   
            ...
            if ((pbuf_hdr->IP_AddrDest & NET_IP_ADDR_CLASS_A_MASK) == NET_IP_ADDR_CLASS_A) {
                                                                    /* Chk Class-A 'This Host'    (see Note #9d1B2a).   */
                ...
                if ((pbuf_hdr->IP_AddrDest & NET_IP_ADDR_CLASS_A_MASK_HOST) ==
                    (NET_IP_ADDR_BROADCAST & NET_IP_ADDR_CLASS_A_MASK_HOST)) {
                     ip_broadcast = DEF_YES;
                                                                    ...
                                                                    /* Chk Class-B 'This Host'    (see Note #9d1B2a).   */
                ...
                if ((pbuf_hdr->IP_AddrDest & NET_IP_ADDR_CLASS_B_MASK_HOST) ==
                    (NET_IP_ADDR_BROADCAST & NET_IP_ADDR_CLASS_B_MASK_HOST)) {
                     ip_broadcast = DEF_YES;
                                                                    ...
                                                                    /* Chk Class-C 'This Host'    (see Note #9d1B2a).   */
...
*
*                       (2) Total Length
*                       (3) Type of Service (TOS)                                   See Note #2c
*                       (4) Source      Address                                     See Note #2d
...
*                           (B) (1)                           MUST       be one of the following :
*                                   (a) This      host's                    RFC #1122, Section 3.2.1.3.(1)
*                                   (b) This      Host                      RFC #1122, Section 3.2.1.3.(a)
*                                   (c) Specified Host                      RFC #1122, Section 3.2.1.3.(b)
*
*                               (2)                           MUST   NOT be one of the following :
*                                   (a) Limited   Broadcast                 RFC #1122, Section 3.2.1.3.(c)
*                                   (b) Directed  Broadcast                 RFC #1122, Section 3.2.1.3.(d)
...
/*$PAGE*/
                                                                    /* ------------- VALIDATE IP SRC ADDR ------------- */
                                                                    /* See Note #1a4.                                   */
    if (addr_src != NetIP_AddrThisHost) {                           /* Chk this host's addr    (see Note #2d1B1a).      */

        if (addr_src != NET_IP_ADDR_THIS_HOST) {                    /* Chk 'This Host'         (see Note #2d1B1b).      */

            if ((addr_src & NET_IP_ADDR_CLASS_A_MASK) == NET_IP_ADDR_CLASS_A) {
                                                                    /* Chk Class-A 'This Host' (see Note #2d1B1b).      */
                if ((addr_src              & NET_IP_ADDR_CLASS_A_MASK_HOST) !=
                    ...
                    *perr = NET_IP_ERR_INVALID_ADDR_SRC;
                     return;
                                                                    ...
                                                                    /* Chk Class-B 'This Host' (see Note #2d1B1b).      */
                if ((addr_src              & NET_IP_ADDR_CLASS_B_MASK_HOST) !=
                    ...
                    *perr = NET_IP_ERR_INVALID_ADDR_SRC;
                     return;
                                                                    ...
                                                                    /* Chk Class-C 'This Host' (see Note #2d1B1b).      */
...
/*$PAGE*/
                                                                    /* ------------- VALIDATE IP DEST ADDR ------------ */
                                                                    /* See Note #1a5.                                   */
                                                                    /* Chk invalid 'This Host'      (see Note #2e1B2a). */
    if (addr_dest == NET_IP_ADDR_THIS_HOST) {
        NET_CTR_ERR_INC(NetIP_ErrTxHdrAddrDestCtr);
       *perr = NET_IP_ERR_INVALID_ADDR_DEST;
        return;

                                                                    /* Chk invalid localhost addrs  (see Note #2e1B1a). */
        ...
        if ((addr_dest             & NET_IP_ADDR_LOCAL_HOST_MASK_HOST) == 
            ...
            *perr = NET_IP_ERR_INVALID_ADDR_DEST;
             return;
                                                                    ...
                                                                    /* Chk link-local 'This Host'   (see Note #2e1B2a). */
        ...
        if ((addr_dest & NET_IP_ADDR_CLASS_A_MASK) == NET_IP_ADDR_CLASS_A) { 
            ...
            if ((addr_dest             & NET_IP_ADDR_CLASS_A_MASK_HOST) ==
                ...
                *perr = NET_IP_ERR_INVALID_ADDR_DEST;
                 return;
                                                                    ...
                                                                    /* Chk Class-B 'This Host'      (see Note #2e1B2a). */
            if ((addr_dest             & NET_IP_ADDR_CLASS_B_MASK_HOST) ==
                ...
                *perr = NET_IP_ERR_INVALID_ADDR_DEST;
                 return;
                                                                    ...
                                                                    /* Chk Class-C 'This Host'      (see Note #2e1B2a). */
           ...
           *perr = NET_IP_ERR_INVALID_ADDR_DEST;
            return;
                                                                    ...
                                                                    /* Chk subnet 'This Host'       (see Note #2e1B2a). */
        if ((NetIP_AddrThisHostSubnetMask     != NET_IP_ADDR_NONE) &&
            (NetIP_AddrThisHostSubnetMaskHost != NET_IP_ADDR_NONE) &&
            (NetIP_AddrThisHostSubnetNet      != NET_IP_ADDR_NONE)) {
...
*                           (A) This host's address                     See 'net_ip.c  Note #1a'
...
*                               NET_IP_ERR_TX_DEST_LOCAL_HOST       Destination is localhost.
...
*
*               (3) (a) RFC #1122, Section 3.3.1 states that "the IP layer chooses the correct next hop 
*                       for each datagram it sends" :
*
*                       (1) "If the destination is on a connected network, the datagram is sent directly 
*                            to the destination host;" ...
*                       (2) "Otherwise, it has to be routed to a gateway on a connected network."
*
                                           ...
                                           NET_ERR      *perr)
{
    ...
    NET_IP_ADDR  addr_dest;
                                                                ...
                                                                /* ---------------- GET IP TX PKT ADDRS --------------- */
    addr_src  = (NET_IP_ADDR)pbuf_hdr->IP_AddrSrc;
    addr_dest = (NET_IP_ADDR)pbuf_hdr->IP_AddrDest;


                                                                /* --------------- CHK THIS HOST'S ADDR --------------- */
    if (addr_dest == NetIP_AddrThisHost) {
        if (NetIP_AddrThisHost != NET_IP_ADDR_NONE) {           /* If valid host addr cfg'd, tx to this host's addr.    */
            NET_CTR_STAT_INC(NetIP_StatTxDestThisHostCtr);
            pbuf_hdr->IP_AddrNextRoute = (CPU_INT32U)addr_dest;
        ...
        NET_CTR_STAT_INC(NetIP_StatTxDestLocalHostCtr);
        pbuf_hdr->IP_AddrNextRoute = (CPU_INT32U)addr_dest;
       *perr                       =  NET_IP_ERR_TX_DEST_LOCAL_HOST;

                                                                /* --------------- CHK LIM'D BROADCAST ---------------- */
    } else if (addr_dest == NET_IP_ADDR_BROADCAST) {            /* Chk lim'd broadcast (see #3b2a).                     */
        NET_CTR_STAT_INC(NetIP_StatTxDestBroadcastCtr);
        NET_CTR_STAT_INC(NetIP_StatTxDestLocalNetCtr);
        DEF_BIT_SET(pbuf_hdr->Flags, NET_BUF_FLAG_BROADCAST_TX);
        pbuf_hdr->IP_AddrNextRoute = (CPU_INT32U)addr_dest;
       *perr                       =  NET_IP_ERR_TX_DEST_BROADCAST;

                                                                /* --------------- CHK LINK-LOCAL ADDRS --------------- */
                                                                /* Chk link-local src  addr (see #3c1A).                */
    } else if ((addr_src  & NET_IP_ADDR_LOCAL_LINK_MASK_NET) == NET_IP_ADDR_LOCAL_LINK_NET) {
        NET_CTR_STAT_INC(NetIP_StatTxDestLocalLinkCtr);
        pbuf_hdr->IP_AddrNextRoute = (CPU_INT32U)addr_dest;
       *perr                       =  NET_IP_ERR_TX_DEST_HOST_THIS_NET;
                                                                /* Chk link-local dest addr (see #3c1B).                */
    } else if ((addr_dest & NET_IP_ADDR_LOCAL_LINK_MASK_NET) == NET_IP_ADDR_LOCAL_LINK_NET) {
                                                                /* Chk link-local broadcast (see #3c3B).                */
        if ((addr_dest             & NET_IP_ADDR_LOCAL_LINK_MASK_HOST) ==
            (NET_IP_ADDR_BROADCAST & NET_IP_ADDR_LOCAL_LINK_MASK_HOST)) {
             NET_CTR_STAT_INC(NetIP_StatTxDestBroadcastCtr);
             DEF_BIT_SET(pbuf_hdr->Flags, NET_BUF_FLAG_BROADCAST_TX);
...
/*$PAGE*/
        ...
        if (NetIP_AddrDfltGateway != NET_IP_ADDR_NONE) {                    /* If valid  dflt gateway cfg'd, ...        */
            NET_CTR_STAT_INC(NetIP_StatTxDestRemoteNetCtr);
            pbuf_hdr->IP_AddrNextRoute = (CPU_INT32U)NetIP_AddrDfltGateway; /*... tx via dflt gateway (see Note #3a2).  */
