...
void  NetTmr_Init (NET_ERR  *perr)
{
    ...
    if (*perr != NET_OS_ERR_NONE) {
         return;
                                                                ...
                                                                /* ------------------ INIT TMR POOL ------------------- */
    NetTmr_PoolPtr = (NET_TMR *)0;                              /* Init-clr net tmr pool (see Note #2).                 */
...
*                           (A) Execute timer's expiration function
...
void  NetTmr_TaskHandler (void)
{
    ...
    while (NetTmr_TaskListPtr != (NET_TMR *)0) {                        /* Handle  Tmr Task List tmrs (see Note #5a).   */
        ...
        NetTmr_TaskListPtr = (NET_TMR *)NetTmr_TaskListPtr->NextPtr;    /* Set next tmr to update (see Note #6a1).      */
...
#if  (NET_ERR_CFG_ARG_CHK_DBG_EN == DEF_ENABLED)                        /* Clr tmr task ptr (see Note #6b1).            */
    NetTmr_TaskListPtr = (NET_TMR *)0;
#endif
